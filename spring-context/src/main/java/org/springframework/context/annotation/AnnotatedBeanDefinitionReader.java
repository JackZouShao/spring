...
public class AnnotatedBeanDefinitionReader {
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		// 1. 解析将beanClass代表的bean封装成beanDefinition

		// abd 即是beanDefinition 因为
		// AnnotatedGenericBeanDefinition 继承 GenericBeanDefinition类 实现 AnnotatedBeanDefinition接口
		// AnnotatedBeanDefinition 实现 BeanDefinition接口
		...
		// ScopeMetadata主要用来描述bean的Scope特性，包括scopeName(singleton还是prototype)和scope代理行为
		// bean的Scope属性默认是单列（singleton）
		...
		// 执行@Scope代理行为，返回执行后的definitionHolder
		definitionHolder = AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);

		// 2.注册将生成的beanDefinition put到Map中
